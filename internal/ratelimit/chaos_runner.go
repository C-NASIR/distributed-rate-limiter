// Package ratelimit provides chaos testing helpers.
package ratelimit
...
func NewChaosHarness() (*ChaosHarness, error) {
	...
	membership := NewSingleInstanceMembership("node", "local")
	ruleDB := NewInMemoryRuleDB(nil)
	outbox := NewInMemoryOutbox()
	pubsub := newChaosPubSub(NewInMemoryPubSub())
...
func scenarioRedisRecover() ChaosScenario {
	return ChaosScenario{
		...
		Run: func(ctx context.Context, h *ChaosHarness) error {
			...
			return waitForMode(ctx, h.App, ModeNormal, 2*time.Second)
		},
	}
}
	...
	return ChaosScenario{
		...
		Run: func(ctx context.Context, h *ChaosHarness) error {
			...
			for i := 0; i < ruleCount; i++ {
				...
				rule, err := h.App.AdminHandler.CreateRule(ctx, &CreateRuleRequest{
					TenantID:  tenantID,
					Resource:  resource,
			...
			for round := 0; round < 5; round++ {
				for i, rule := range rules {
					updated, err := h.App.AdminHandler.UpdateRule(ctx, &UpdateRuleRequest{
...
func scenarioPubSubRepair() ChaosScenario {
	return ChaosScenario{
		...
		Run: func(ctx context.Context, h *ChaosHarness) error {
			...
			updated, err := h.RuleDB.Update(ctx, &UpdateRuleRequest{
...
func waitForMode(ctx context.Context, app *Application, target OperatingMode, timeout time.Duration) error {
	if app == nil {
		return errors.New("app is required")
	}
	if ctx == nil {
...
func waitForRuleCache(ctx context.Context, app *Application, db RuleDB, timeout time.Duration) error {
...
func ruleCacheMatches(ctx context.Context, cache *RuleCache, db RuleDB) bool {
	if cache == nil || db == nil {
		return false
	}
	if ctx == nil {
...
func newChaosPubSub(inner *InMemoryPubSub) *chaosPubSub {
	if inner == nil {
		inner = NewInMemoryPubSub()
	}
