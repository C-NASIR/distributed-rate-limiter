...
func TestHandler_MetricsIncludeRegion(t *testing.T) {
	...
	app, err := NewApplication(&config.Config{Region: "r1"})
	if err != nil {
		t.Fatalf("unexpected app error: %v", err)
	}
	app.RuleCache.ReplaceAll([]*core.Rule{{
		TenantID:  "tenant",
		Resource:  "resource",
	...
	resp, err := app.RateLimitHandler.CheckLimit(context.Background(), &core.CheckLimitRequest{
		TenantID: "tenant",
		Resource: "resource",
		UserID:   "user",
		Cost:     1,
