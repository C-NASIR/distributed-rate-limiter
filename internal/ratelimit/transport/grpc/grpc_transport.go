// Package grpctransport provides a gRPC transport.
...
// GRPCTransportConfig configures the gRPC transport.
...
func NewGRPCTransport(addr string, ready func() bool, mode func() core.OperatingMode, cfg GRPCTransportConfig) *GRPCTransport {
	if addr == "" {
		addr = ":9090"
	}
	if ready == nil {
	...
	if mode == nil {
		mode = func() core.OperatingMode { return core.ModeNormal }
	}
	if cfg.KeepAlive <= 0 {
...
func (t *GRPCTransport) ServeRateLimit(service core.RateLimitService) error {
	if service == nil {
...
func (t *GRPCTransport) ServeAdmin(service core.AdminService) error {
	if service == nil {
...
// SetListener overrides the listener (useful for tests).
func (t *GRPCTransport) SetListener(listener net.Listener) {
	if t == nil {
		return
	}
	t.mu.Lock()
...
func (s *grpcRateLimitServer) CheckLimit(ctx context.Context, req *ratelimitv1.CheckLimitRequest) (*ratelimitv1.CheckLimitResponse, error) {
	...
	resp, err := s.service.CheckLimit(ctx, &core.CheckLimitRequest{
	...
	if err != nil {
		...
		if code == core.CodeInvalidInput || code == core.CodeInvalidCost {
...
func (s *grpcRateLimitServer) CheckLimitBatch(ctx context.Context, req *ratelimitv1.BatchCheckLimitRequest) (*ratelimitv1.BatchCheckLimitResponse, error) {
	...
	requests := make([]*core.CheckLimitRequest, len(req.GetRequests()))
...
func (s *grpcAdminServer) CreateRule(ctx context.Context, req *ratelimitv1.CreateRuleRequest) (*ratelimitv1.Rule, error) {
	...
	rule, err := s.service.CreateRule(ctx, &core.CreateRuleRequest{
		TenantID:       req.GetTenantId(),
		Resource:       req.GetResource(),
...
func (s *grpcAdminServer) UpdateRule(ctx context.Context, req *ratelimitv1.UpdateRuleRequest) (*ratelimitv1.Rule, error) {
	...
	rule, err := s.service.UpdateRule(ctx, &core.UpdateRuleRequest{
		TenantID:        req.GetTenantId(),
		Resource:        req.GetResource(),
		Algorithm:       req.GetAlgorithm(),
		Limit:           req.GetLimit(),
...
func (s *grpcHealthServer) Mode(context.Context, *ratelimitv1.HealthRequest) (*ratelimitv1.HealthResponse, error) {
	...
	return &ratelimitv1.HealthResponse{Status: modeStatus(mode)}, nil
}

func modeStatus(mode core.OperatingMode) string {
	switch mode {
...
func toGRPCCheckLimitResponse(resp *core.CheckLimitResponse) *ratelimitv1.CheckLimitResponse {
	if resp == nil {
		return &ratelimitv1.CheckLimitResponse{}
	}
	return &ratelimitv1.CheckLimitResponse{
...
func toGRPCRule(rule *core.Rule) *ratelimitv1.Rule {
	if rule == nil {
...
func grpcErrorCode(code core.ErrorCode) string {
	if code != "" {
		return string(code)
	}
