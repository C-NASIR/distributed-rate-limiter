// Package core provides the rate limit handler.
package core
...
func NewRateLimitHandler(rules *RuleCache, pool *LimiterPool, keys *KeyBuilder, region string, rp *ResponsePool, tracer observability.Tracer, sampler observability.Sampler, metrics observability.Metrics, coalescer *Coalescer) *RateLimitHandler {
	if tracer == nil {
		tracer = observability.NoopTracer{}
	}
	if sampler == nil {
		sampler = observability.NewHashSampler(100)
	}
	if metrics == nil {
...
// SetBatchPlanner overrides the batch planner.
func (h *RateLimitHandler) SetBatchPlanner(planner *BatchPlanner) {
	if h == nil || planner == nil {
		return
	}
	h.batch = planner
}

// SetInFlight overrides the inflight tracker.
func (h *RateLimitHandler) SetInFlight(inflight *InFlight) {
	if h == nil {
		return
	}
	h.inflight = inflight
...
// SetRequestTimeout overrides the request timeout.
func (h *RateLimitHandler) SetRequestTimeout(timeout time.Duration) {
	if h == nil {
		return
	}
