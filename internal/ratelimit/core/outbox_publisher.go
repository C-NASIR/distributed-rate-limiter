// Package core publishes outbox events.
package core
...
func NewOutboxPublisher(outbox Outbox, pubsub PubSub, channel string, interval time.Duration) *OutboxPublisher {
...
// SetInterval overrides the publishing interval.
func (p *OutboxPublisher) SetInterval(interval time.Duration) {
	if p == nil {
		return
	}
	p.interval = interval
