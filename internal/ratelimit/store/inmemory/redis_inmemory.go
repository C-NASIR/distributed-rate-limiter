// Package inmemory provides an in-memory Redis implementation.
package inmemory
...
func (r *InMemoryRedis) Pipeline() core.RedisPipeline {
	if r == nil {
...
func (r *InMemoryRedis) ExecTokenBucket(ctx context.Context, key string, params core.RuleParams, cost int64) (*core.Decision, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *InMemoryRedis) ExecFixedWindow(ctx context.Context, key string, params core.RuleParams, cost int64) (*core.Decision, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *InMemoryRedis) ExecSlidingWindow(ctx context.Context, key string, params core.RuleParams, cost int64) (*core.Decision, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (p *InMemoryPipeline) ExecTokenBucket(key string, params core.RuleParams, cost int64) {
...
func (p *InMemoryPipeline) ExecFixedWindow(key string, params core.RuleParams, cost int64) {
...
func (p *InMemoryPipeline) ExecSlidingWindow(key string, params core.RuleParams, cost int64) {
	p.ops = append(p.ops, redisOp{kind: opSlidingWindow, key: key, params: params, cost: cost})
}

// Exec runs queued operations.
func (p *InMemoryPipeline) Exec(ctx context.Context) ([]*core.Decision, error) {
	if p.redis == nil {
	...
	decisions := make([]*core.Decision, len(p.ops))
...
func (r *InMemoryRedis) execTokenBucketLocked(key string, params core.RuleParams, cost int64) (*core.Decision, error) {
	if !r.healthy.Load() {
...
func (r *InMemoryRedis) execFixedWindowLocked(key string, params core.RuleParams, cost int64) (*core.Decision, error) {
	if !r.healthy.Load() {
		return nil, errors.New("redis unhealthy")
	}
	if cost <= 0 || params.Limit <= 0 {
...
func (r *InMemoryRedis) execSlidingWindowLocked(key string, params core.RuleParams, cost int64) (*core.Decision, error) {
	if !r.healthy.Load() {
		return nil, errors.New("redis unhealthy")
	}
	if cost <= 0 || params.Limit <= 0 {
